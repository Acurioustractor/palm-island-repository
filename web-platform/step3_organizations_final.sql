...
-- Add constraint for organization type if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'organizations_type_check') THEN
    ALTER TABLE organizations ADD CONSTRAINT organizations_type_check CHECK (
      type IN (
        'aboriginal_community', 'torres_strait_islander', 'indigenous_ngo',
        'government_service', 'healthcare', 'education', 'environmental',
        'social_services', 'arts_culture', 'economic_development', 'other', 'community', 'philanthropy'
      )
    );
  END IF;
END $$;
...
-- Organizations - using existing column names
CREATE INDEX IF NOT EXISTS organizations_type_idx ON organizations(type);
CREATE INDEX IF NOT EXISTS organizations_location_idx ON organizations(location);
...
-- ============================================================================
-- SEED DATA - Update existing PICC organization and add services
-- ============================================================================

-- Update existing PICC organization with additional fields
UPDATE organizations 
SET
  legal_name = 'Palm Island Community Company Limited',
  short_name = 'PICC',
  type = 'aboriginal_community',
  tagline = 'Our Community, Our Future, Our Way',
  mission_statement = 'Delivering culturally appropriate, community-controlled services that strengthen Palm Island families, preserve culture, and build a thriving future for all generations.',
  traditional_country = 'Manbarra Country',
  language_groups = ARRAY['Manbarra', 'Bwgcolman'],
  website = 'https://picc.com.au',
  email = 'info@picc.com.au',
  phone = '+61 7 4770 1177',
  established_date = '1999-01-01',
  indigenous_controlled = TRUE,
  governance_model = 'community_controlled',
  empathy_ledger_enabled = TRUE,
  annual_reports_enabled = TRUE,
  impact_tracking_enabled = TRUE,
  has_cultural_protocols = TRUE,
  elder_approval_required = TRUE,
  default_story_access_level = 'community'
...
DECLARE
  picc_org_id UUID;
BEGIN
  -- Get existing PICC org by name
  SELECT id INTO picc_org_id FROM organizations WHERE name = 'Palm Island Community Company';
