...
export function StoryInfobox({ data, className = '' }: StoryInfoboxProps) {
  const getSensitivityBadge = (level?: string) => {
    const badges = {
      low: { color: 'bg-green-100 text-green-800', label: 'General Content' },
      medium: { color: 'bg-yellow-100 text-yellow-800', label: 'Cultural Awareness' },
      high: { color: 'bg-orange-100 text-orange-800', label: 'Cultural Sensitive' },
      restricted: { color: 'bg-red-100 text-red-800', label: 'Restricted' },
    };
  ...
  const getAccessBadge = (level: string) => {
    const badges = {
      ...
      community: { color: 'bg-purple-100 text-purple-800', label: 'Community Only' },
      restricted: { color: 'bg-red-100 text-red-800', label: 'Restricted' },
    };
  ...
  return (
    <div className={`bg-gradient-to-br from-blue-50 to-teal-50 rounded-lg border-2 border-blue-200 overflow-hidden ${className}`}>
      {/* Header */}
