 ...
 * High-quality stories marked as featured - ONE PER STORYTELLER
 */
export async function getFeaturedStories(limit: number = 6): Promise<Story[]> {
  ...
  const { data, error } = await supabase
    ...
    .limit(limit * 3); // Get more to filter for unique storytellers
  ...
  // Filter to get ONE story per storyteller
  const seenStorytellers = new Set<string>();
  const uniqueFeaturedStories: Story[] = [];

  for (const story of (data || [])) {
    const storytellerId = story.storyteller?.id;
    if (storytellerId && !seenStorytellers.has(storytellerId)) {
      seenStorytellers.add(storytellerId);
      uniqueFeaturedStories.push(story as Story);
      if (uniqueFeaturedStories.length >= limit) break;
    } else if (!storytellerId) {
      // Include stories without storytellers
      uniqueFeaturedStories.push(story as Story);
      if (uniqueFeaturedStories.length >= limit) break;
    }
  }

  return uniqueFeaturedStories;
}
...
/**
 * Get elder stories
 * Stories from elders and cultural advisors - ONE PER ELDER
 */
export async function getElderStories(limit: number = 6): Promise<Story[]> {
  ...
  // Filter to get ONE story per elder
  const seenStorytellers = new Set<string>();
  const uniqueElderStories: Story[] = [];

  for (const story of (data || [])) {
    const storytellerId = story.storyteller?.id;
    if (storytellerId && !seenStorytellers.has(storytellerId)) {
      seenStorytellers.add(storytellerId);
      uniqueElderStories.push(story as Story);
      if (uniqueElderStories.length >= limit) break;
    }
  }
