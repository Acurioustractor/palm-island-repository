...
/**
 * Batch analyze all Elder interviews and extract citation-backed quotes.
 *
 * Requires:
...
function normalizeExtractedQuoteSentiment(input) {
...
function looksLikeTimecode(text) {
  const s = String(text || '')
  if (!s) return false
...
function looksLikeEncryptedTranscript(input) {
  ...
  // Fernet tokens commonly start with "gAAAA" and are URL-safe base64.
...
function normalizeAndFilterQuotes(quotes) {
  ...
  for (const q of Array.isArray(quotes) ? quotes : []) {
    ...
    cleaned.push({
      ...
      sentiment: normalizeExtractedQuoteSentiment(q?.sentiment),
...
async function countExistingQuotesForInterview(interviewId) {
  ...
  // Prefer extracted_quotes.interview_id when present; fall back to legacy metadata->>interview_id.
  try {
    ...
    if (!direct?.error) {
      ...
      if (c > 0) return c
      // If zero, also check legacy rows (older inserts stored interview_id in metadata only).
    } else {
      ...
      if (missingCol && missingCol !== 'interview_id') {
        // Schema mismatch unrelated to interview_id.
        return 0
      }
    }
  } catch {
...
async function main() {
  ...
  const { data: elders, error: eldersError } = await supabase
    .from('profiles')
    .select('id, full_name, preferred_name, is_elder')
    .eq('is_elder', true)
  ...
  console.log(`Found ${elderRows.length} elders${LIMIT ? ` (limit=${LIMIT})` : ''}. Starting analysis...`)

  for (const elder of elderRows) {
    const elderEntry = { id: elder.id, name: displayName(elder), interviews: [] }
    report.elders.push(elderEntry)
    ...
    if (interviewsError) {
      ...
      elderEntry.interviews.push({ error: String(interviewsError.message || interviewsError) })
      continue
    }

    const rows = Array.isArray(interviews) ? interviews : []
    report.totals.interviews += rows.length
    ...
    for (const interview of rows) {
      ...
      elderEntry.interviews.push(item)
      ...
      if (looksLikeEncryptedTranscript(transcript)) {
        item.action = 'skipped_encrypted_transcript'
        item.error = 'Transcript looks encrypted (fernet-like token); cannot analyze until decrypted'
        report.totals.skipped++
        continue
      }
      ...
      if (interview?.requires_elder_approval === true && !interview?.approved_at) {
        item.action = 'skipped_needs_elder_approval'
        report.totals.skipped++
        continue
      }
      ...
      // Avoid duplicates if we already have quotes for this interview.
      if (!FORCE) {
      ...
      console.log(`Analyzing: ${elderEntry.name} — ${interviewTitle} (${interview.id})`)
      ...
      try {
        ...
        const response = await anthropic.messages.create({
          ...
          messages: [
            {
              role: 'user',
              content: `Analyze these interview transcript segments from ${elderEntry.name} at Palm Island Community Company. Extract meaningful, print-ready quotes for annual reports, community stories, and immersive story experiences.
...
Return ONLY the JSON, no other text.`,
        ...
        // Upsert segments for provenance
        ...
        try {
          ...
          if (citations.length > 0) {
            await supabase.from('extracted_quote_citations').insert(citations)
          }
        } catch {
          // optional feature
        }

        // Store review artifact snapshot (best-effort)
        try {
          await supabase.from('interview_review_artifacts').insert({
            interview_id: interview.id,
            profile_id: elder.id,
            artifact_type: 'quote_library',
            title: `Quote library: ${elderEntry.name} — ${interviewTitle}`,
            status: 'draft',
            visibility: 'internal',
            content: {
        ...
        // Update interview fields (best-effort; avoid clobbering metadata if present)
        try {
          ...
          if (updateRes?.error) {
            // If status constraint differs, retry without changing status.
            await supabase
              .from('interviews')
              .update({
                key_themes: analysis.key_themes,
        ...
        report.totals.errors++
        console.error(`Error analyzing ${elderEntry.name} — ${interviewTitle}: ${item.error}`)
      }
    }
  }
  ...
  const outPath = path.join(outDir, `elder-interview-analysis-${startedAt.replace(/[:.]/g, '-')}.json`)
